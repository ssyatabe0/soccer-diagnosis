...
type CaseAsset = { id: string; customer_id: string | null; case_code: string | null; full_name: string | null; parent_name: string | null; child_name: string | null; problem: string | null; cause: string | null; improvement: string | null; result: string | null; publish_status: string }
type Content = { id: string; source_id: string | null; content_type: string; title: string; body: string; status: string; created_at: string }
...
async function loadData() {
  ...
  const [cases, contents] = await Promise.all([
    ...
    supabase.from('generated_contents').select('*').in('content_type', ['blog_draft', 'case_article', 'seo_article']).order('created_at', { ascending: false }).limit(100),
  ])
...
export default async function AiSecretaryArticleGeneratorPage({ searchParams }: { searchParams?: Promise<SearchParams> | SearchParams }) {
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 className="text-2xl font-black text-gray-900">AI記事生成</h2>
          <p className="mt-1 text-sm text-gray-500">症例カルテからブログ下書き・症例記事・SEO記事を作ります。公開はしません。</p>
        </div>
          ...
          {cases.map((item) => (
            <article key={item.id} className="rounded-xl border border-gray-100 bg-gray-50 p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div><h4 className="font-black text-gray-900">{nameOf(item)}</h4><p className="mt-2 text-sm leading-7 text-gray-700">{item.problem || item.result || '-'}</p></div>
                <div className="flex flex-wrap gap-2"><CaseContentGenerateButton token={token} caseId={item.id} contentType="blog_draft" label="ブログ下書き" /><CaseContentGenerateButton token={token} caseId={item.id} contentType="case_article" label="症例記事" /><CaseContentGenerateButton token={token} caseId={item.id} contentType="seo_article" label="SEO記事" /></div>
              </div>
            </article>
          ))}
